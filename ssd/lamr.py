...
def meanAP_LogAverageMissRate():
    ...
    bbb.filter_ignore(ground_truth, [bbb.ClassLabelFilter(['person']),  # only consider 'person' objects
                                     bbb.HeightRangeFilter((50, float('Inf'))), 
...
if __name__ == '__main__':
    ...
    for name in tqdm(names):
        ...
        for line in lines:
            anns = line[:-1].split(' ')
            detection_results.append({"image_id": name[:-4].replace('_', '/').replace('/lwir/', '/'), "category_id": 1, "bbox":[float(anns[2]), float(anns[3]), float(anns[4]) - float(anns[2]), float(anns[5]) - float(anns[3])], "score": float(anns[1])})
