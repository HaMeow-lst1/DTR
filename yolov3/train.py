...
from utils.utils_fit import fit_one_epoch_stage1, fit_one_epoch_stage2, fit_one_epoch_stage3
import DTR_config
...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
    classes_path    = 'model_data/' + dtr.dataset + '.txt'
    #---------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = dtr.epoch1 + dtr.epoch2 + dtr.epoch3
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch == dtr.epoch1: #stage2
                for param in model.backbone.parameters():
                    param.requires_grad = False
            if epoch == dtr.epoch1 + dtr.epoch2: #stage3
                for param in model.backbone.parameters():
                    param.requires_grad = True
                for param in model.fdnet.parameters():
                    param.requires_grad = False
            
            if epoch < dtr.epoch1:
                fit_one_epoch_stage1(model_train, model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)
            elif epoch < dtr.epoch1 + dtr.epoch2:
                fit_one_epoch_stage2(model_train, model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)
            else:
                fit_one_epoch_stage3(model_train, model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)
